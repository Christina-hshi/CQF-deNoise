...
#define BITS_PER_SLOT 0
  ...
  //bool qf_spin_lock(QF *cf, volatile int *lock, uint64_t idx, bool flag_spin);
  bool qf_spin_lock(volatile int *lock, bool flag_spin);
  void qf_spin_unlock(volatile int *lock);
  //CHRISTINA
  ...
  bool qf_insert_advance(QF *qf, uint64_t key, uint64_t value, uint64_t count, bool lock, bool spin, bool& isNew);
